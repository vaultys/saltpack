const { signcrypt, designcrypt, SigncryptStream, DesigncryptStream, debugSetKey, debugSetKeypair } = require("../dist/signcryption");
const { SymmetricKeyRecipient } = require("../dist/signcryption/recipient");
...
const { KEYPAIR, /*KEYPAIR_ALICE,*/ KEYPAIR_BOB, KEYPAIR_MALLORY, SYMMETRIC_KEY_BOB, SYMMETRIC_KEY_MALLORY } = require("./data/encryption-keys");
const { KEYPAIR_ALICE /*, KEYPAIR_BOB as KEYPAIR_BOB_S */ } = require("./data/signing-keys");
const { SIGNCRYPTED, SIGNCRYPTED_SYMMETRIC } = require("./data/signcryption-tests");
...
test("encrypt", async () => {
  const encrypted = await signcrypt(INPUT_STRING, KEYPAIR_ALICE, [KEYPAIR_BOB.publicKey]);

  expect(encrypted).toStrictEqual(SIGNCRYPTED);
});

test("encrypt stream", async () => {
  const stream = new SigncryptStream(KEYPAIR_ALICE, [KEYPAIR_BOB.publicKey]);
...
describe("symmetric key recipients", () => {
  test("encrypt for symmetric key", async () => {
    const encrypted = await signcrypt(INPUT_STRING, KEYPAIR_ALICE, [new SymmetricKeyRecipient(Buffer.alloc(32), SYMMETRIC_KEY_BOB)]);

    expect(encrypted).toStrictEqual(SIGNCRYPTED_SYMMETRIC);
  });

  test("decrypt with symmetric key", async () => {
    const data = await designcrypt(SIGNCRYPTED_SYMMETRIC, new SymmetricKeyRecipient(Buffer.alloc(32), SYMMETRIC_KEY_BOB), KEYPAIR_ALICE.publicKey);
  ...
  test("encrypt stream with symmetric key", async () => {
    const stream = new SigncryptStream(KEYPAIR_ALICE, [new SymmetricKeyRecipient(Buffer.alloc(32), SYMMETRIC_KEY_BOB)]);
  ...
  test("decrypt stream with symmetric key", async () => {
    const stream = new DesigncryptStream(new SymmetricKeyRecipient(Buffer.alloc(32), SYMMETRIC_KEY_BOB), KEYPAIR_ALICE.publicKey);
  ...
  test("decrypt with wrong key fails", async () => {
    await expect(async () => {
      await designcrypt(SIGNCRYPTED, new SymmetricKeyRecipient(Buffer.alloc(32), SYMMETRIC_KEY_MALLORY));
    }).rejects.toThrow();
  });
});
