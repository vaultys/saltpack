...
export default class SignedMessagePayload {
  ...
  static create(header: SignedMessageHeader, private_key: Uint8Array, data: Buffer, index: number | bigint, final = false): SignedMessagePayload {
    if (typeof index === "number") index = BigInt(index);

    const sign_data = this.generateSignData(header.hash, index, final, data);
    const signature = sign.detached(Uint8Array.from(sign_data), Uint8Array.from(private_key));
  ...
  static encodePayload(final: boolean, signature: Uint8Array, payload_chunk: Uint8Array): Buffer {
    return Buffer.from(encode([final, signature, payload_chunk]));
  }
  ...
  verify(header: SignedMessageHeader, public_key: Uint8Array, index: bigint) {
    ...
    if (!sign.detached.verify(Uint8Array.from(sign_data), Uint8Array.from(this.signature), Uint8Array.from(public_key))) {
