...
export default class EncryptedMessageHeader extends Header {
  ...
  static create(public_key: Uint8Array, payload_key: Uint8Array, sender_public_key: Uint8Array, recipients: EncryptedMessageRecipient[]) {
    if (!isBufferOrUint8Array(sender_public_key) || sender_public_key.length !== 32) {
    ...
    const sender_secretbox = secretbox(Uint8Array.from(sender_public_key), Uint8Array.from(EncryptedMessageHeader.SENDER_KEY_SECRETBOX_NONCE), Uint8Array.from(payload_key));
  ...
  static encodeHeader(public_key: Uint8Array, sender: Uint8Array, recipients: EncryptedMessageRecipient[]): [Buffer, Buffer] {
    const data = [
      "saltpack",
      [2, 0],
  ...
  static decode(encoded: Uint8Array, unwrapped = false) {
    ...
    return new this(
      public_key as any,
      sender_secretbox as any,
      recipients.map((recipient: unknown, index) => {
   ...
   */
  decryptPayloadKey(keypair: BoxKeyPair): [Uint8Array, EncryptedMessageRecipient] {
    // 5. Precompute the ephemeral shared secret using crypto_box_beforenm with the ephemeral public key and
    // the recipient's private key.
    const shared_secret = box.before(this.public_key, keypair.secretKey);
  ...
  decryptSender(payload_key: Uint8Array): Uint8Array {
    const sender_public_key = secretbox.open(Uint8Array.from(this.sender_secretbox), Uint8Array.from(EncryptedMessageHeader.SENDER_KEY_SECRETBOX_NONCE), Uint8Array.from(payload_key));
