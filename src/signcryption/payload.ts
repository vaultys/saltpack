...
export default class SigncryptedMessagePayload {
  ...
  static create(header: SigncryptedMessageHeader, payload_key: Uint8Array, private_key: Uint8Array | null, data: Buffer, index: bigint, final = false): SigncryptedMessagePayload {
    ...
    const signature = private_key ? sign.detached(Uint8Array.from(this.generateSignatureData(header.hash, nonce, final, data)), Uint8Array.from(private_key)) : Buffer.alloc(64);
    ...
    const payload_secretbox = Buffer.from(secretbox(Uint8Array.from(Buffer.concat([signature, data])), Uint8Array.from(nonce), Uint8Array.from(payload_key)));
  ...
  static generateSignatureData(header_hash: Uint8Array, nonce: Uint8Array, final: boolean, data: Uint8Array) {
  ...
  decrypt(header: SigncryptedMessageHeader, public_key: Uint8Array | null, payload_key: Uint8Array, index: bigint) {
    ...
    const signature_data = secretbox.open(Uint8Array.from(this.payload_secretbox), Uint8Array.from(nonce), Uint8Array.from(payload_key));
