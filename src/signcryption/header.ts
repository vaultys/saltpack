...
export default class SigncryptedMessageHeader extends Header {
  ...
  static create(public_key: Uint8Array, payload_key: Uint8Array, sender_public_key: Uint8Array | null, recipients: SigncryptedMessageRecipient[]) {
    if (sender_public_key !== null && (!isBufferOrUint8Array(sender_public_key) || sender_public_key.length !== 32)) {
    ...
    const sender_secretbox = secretbox(Uint8Array.from(sender_public_key), Uint8Array.from(SigncryptedMessageHeader.SENDER_KEY_SECRETBOX_NONCE), Uint8Array.from(payload_key));
  ...
  decryptPayloadKeyWithCurve25519Keypair(private_key: Uint8Array): [Uint8Array, SigncryptedMessageRecipient] | null {
    ...
    for (const recipient of this.recipients) {
      const { recipient_identifier, shared_symmetric_key } = SigncryptedMessageRecipient.generateRecipientIdentifierForRecipient(this.public_key, private_key, recipient.recipient_index);
  ...
  decryptSender(payload_key: Uint8Array): Uint8Array | null {
    const sender_public_key = secretbox.open(Uint8Array.from(this.sender_secretbox), Uint8Array.from(SigncryptedMessageHeader.SENDER_KEY_SECRETBOX_NONCE), Uint8Array.from(payload_key));
