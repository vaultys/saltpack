...
export default class SigncryptedMessageRecipient {
  ...
  static create(public_key: Uint8Array, ephemeral_private_key: Uint8Array, payload_key: Uint8Array, index: number | bigint): SigncryptedMessageRecipient {
    ...
    const encrypted_payload_key = secretbox(Uint8Array.from(payload_key), Uint8Array.from(recipient_index), Uint8Array.from(shared_symmetric_key));
  ...
  static createSymmetric(
    ...
    // Secretbox the payload key using this derived symmetric key, with the nonce saltpack_recipsbXXXXXXXX,
    // where XXXXXXXX is the 8-byte big-endian unsigned recipient index.
    const encrypted_payload_key = secretbox(Uint8Array.from(payload_key), Uint8Array.from(recipient_index), Uint8Array.from(derived_key));
  ...
  decryptPayloadKey(shared_symmetric_key: Uint8Array): Uint8Array | null {
    return secretbox.open(Uint8Array.from(this.encrypted_payload_key), Uint8Array.from(this.recipient_index), Uint8Array.from(shared_symmetric_key));
  }
  ...
  static generateRecipientIdentifierForSender(public_key: Uint8Array, ephemeral_private_key: Uint8Array, recipient_index: Uint8Array) {
    ...
    const shared_symmetric_key = Buffer.from(
      box(Uint8Array.from(Buffer.alloc(32).fill("\0")), Uint8Array.from(this.SHARED_KEY_NONCE), Uint8Array.from(public_key), Uint8Array.from(ephemeral_private_key)),
    ).slice(-32);
  ...
  static generateRecipientIdentifierForRecipient(ephemeral_public_key: Uint8Array, private_key: Uint8Array, recipient_index: Uint8Array) {
    ...
    // the last 32 bytes of the resulting box.
    const shared_symmetric_key = Buffer.from(
      box(Uint8Array.from(Buffer.alloc(32).fill("\0")), Uint8Array.from(this.SHARED_KEY_NONCE), Uint8Array.from(ephemeral_public_key), Uint8Array.from(private_key)),
    ).slice(-32);
